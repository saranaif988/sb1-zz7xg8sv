...
export default function HeroSlider() {
  ...
  const slides: MediaSlide[] = [
    ...
    },
    {
      type: "image",
      image:
        "https://ueaiiwmblxyqsflvnzir.supabase.co/storage/v1/object/public/image//decor1.jpg",
  ...
  return (
    <div className="relative h-screen min-h-[672px] overflow-hidden">
      ...
      <button
        onClick={prevSlide}
        className="absolute left-4 top-1/2 -translate-y-1/2 p-3 rounded-full  hover:bg-white/30  transition-all  z-10"
      >
        <ChevronLeft className="h-6 w-6 z-10 text-white" />
      </button>
      <button
        onClick={nextSlide}
        className="absolute right-4 top-1/2 -translate-y-1/2 p-3 rounded-full  hover:bg-white/30 transition-all  z-10"
      >
